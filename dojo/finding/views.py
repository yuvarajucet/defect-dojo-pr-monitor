...
class ViewFinding(View):
    ...
    def update_finding_image_links(self, finding: Finding):
        fields_to_check = [
        'description',
        'request',
        'response',
        ...
        for field_name in fields_to_check:
            ...
            if value and f"/uploads/findings/0/" in value:
                new_value = value.replace(f"/uploads/findings/0/", f"/uploads/findings/{finding.id}/")
                setattr(finding, field_name, new_value)
                updated = True
    ...
    def GetimgcontentDetail(self, finding: Finding):
        output = ''
        output += finding.description
        output += finding.mitigation
        output += finding.impact
        output += finding.steps_to_reproduce
        output += finding.severity_justification
        output += finding.references
        return output


    def MoveImage(self, request: HttpRequest, finding: Finding):
        Exiting_file = FileUpload.objects.filter(origin='findings',origin_id = 0)
        
        if not Exiting_file.exists():
            print(f"no records found")

        body_str = self.GetimgcontentDetail(finding)
        ismoved = False
        for file in Exiting_file:
            if  file.title in body_str:
                path_parts = file.file.name.split('/') 
                old_path = file.file.name
                # Change the folder from old origin_id to new
                path_parts[2] = str(finding.id)
                ...
                print(f"File {file.id}.png moved to the finding id - {finding.id}")
        if ismoved:
            self.update_finding_image_links(finding)
        else:
            print(f"no file moved for origin id - {finding.id}")
    ...
    def get(self, request: HttpRequest, finding_id: int):
        ...
        #move images
        self.MoveImage(request,finding)
...
class EditFinding(View):
    ...
    def RemoveImage(self, request: HttpRequest, finding: Finding):
        Exiting_file = FileUpload.objects.filter(origin='findings',origin_id = int(finding.id))
        ...
        isdeleted = False
        for file in Exiting_file:
            if  file.title not in body_str:
                isdeleted = True
                file.file.delete(save=False)
                file.delete()
                print(f"deleted the file named {file.id}.png since it's not available in the findings")
        if isdeleted is False:
            print(f"no duplicate file found for origin id - {finding.id}")
    ...
    def process_forms(self, request: HttpRequest, finding: Finding, context: dict):
        ...
        #remove unused file to improve storage and cleanup
        self.RemoveImage(request, finding)
        #move images during finding creation
        self.MoveImage(request,finding)
...
class DeleteFinding(View):
    ...
    def process_form(self, request: HttpRequest, finding: Finding, context: dict):
        if context["form"].is_valid():
            ...
            id = finding.id
            #delete all the findings image
            self.RemoveImage(request,finding)
            files = FileUpload.objects.filter(origin='findings',origin_id = int(id))
            for file in files:
                file.file.delete(save=False)  # Deletes the file from storage
                file.delete()  # Deletes the DB entry
            finding.delete()
            # Attempt to delete folder if empty
        try:
            location = default_storage.location
            normalized_location = location.replace("\\", "/")
            os.rmdir(os.path.join(normalized_location , f"uploads/findings/{id}"))
        except OSError:
            print("Folder not empty or doesn't exist")
            pass
