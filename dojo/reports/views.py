...
from dojo.reports.helpers import save_html_report, generate_pdf
import os
...
class CustomReport(View):
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if form.is_valid():
            ...
            html_content = render_to_string(self.get_template(), self.get_context(), request=request)
            save_html_report(html_content, request)
...
def render_custom_report(request) -> HttpResponse:
    file_path = os.path.join(settings.MEDIA_ROOT, "export", "report.html")
    try:
        html_content = None
...
def render_cover_page(request) -> HttpResponse:
    file_path = os.path.join(settings.MEDIA_ROOT, "export", "coverpage.html")
    try:
        html_content = None
        with open(file_path, "r", encoding="utf-8") as f:
            html_content = f.read()
...
def download_custom_report(request: HttpRequest):
