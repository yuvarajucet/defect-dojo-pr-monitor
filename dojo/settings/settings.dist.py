...
env = environ.FileAwareEnv(
    ...
    # PDF export running API server URL
    # This URL helps to generate custom HTML report into custom PDF report.
    DD_PDF_EXPORT_SERVICE_URL=(str, "http://localhost:5073/api/Export/link-to-pdf"),

    # Commit information getting URL from gitea
    DD_COMMIT_INFO_URL = (str, "https://gitea.syncfusion.com/api/v1/repos/{owner}/{repo}/pulls/{pr_id}/commits"),
    GITEA_ACCESS_SECRET = (str, "[access token here]"),
    DEFAULT_PRODUCT_NAME = (str, "NEW_PRODUCT")
)
...
# PDF exporting service url 
PDF_EXPORT_SERVICE = env("DD_PDF_EXPORT_SERVICE_URL")
COMMIT_INFO_GETTING_URL = env("DD_COMMIT_INFO_URL")
GITEA_ACCESS_KEY = env("GITEA_ACCESS_SECRET")
DEFAULT_PRODUCT = env("DEFAULT_PRODUCT_NAME")

# Raises django's ImproperlyConfigured exception if SECRET_KEY not in os.environ
# SECRET_KEY = env("DD_SECRET_KEY")
SECRET_KEY = "IsQwRGIv6m733iG3oN6ocK5QitaVfezyzyOIphrnPF_VfEEKFcy0fWhJtNOeeVoE5pc"
...
if os.getenv("DD_DATABASE_URL") is not None:
    ...
    'default': {
        ...
        'NAME': 'defectdojo',  # or the DB name you created
...
# DOJO_ROOT = env("DD_ROOT") need to change 
DOJO_ROOT = Path(__file__).resolve().parent.parent
# Absolute filesystem path to the directory that will hold user-uploaded files.
# Example: "/var/www/example.com/media/"
# MEDIA_ROOT = env("DD_MEDIA_ROOT")
MEDIA_ROOT = os.path.join(DOJO_ROOT, env("DD_MEDIA_ROOT")) 
...
STATIC_ROOT = env("DD_STATIC_ROOT", default=str(Path(DOJO_ROOT) / "static"))

# URL prefix for static files.
# Example: "http://example.com/static/", "http://static.example.com/"
STATIC_URL = env("DD_STATIC_URL", default="/static/")
...
LOGIN_EXEMPT_URLS = (
    ...
    rf"^{URL_PREFIX}reset/",
    rf"^{URL_PREFIX}giteawebhook/trigger$" # which is used to handle events from gitea
)
