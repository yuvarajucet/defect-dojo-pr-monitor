...
def upload_image(request):
    if request.method == "POST" and request.FILES.get("image"):
        ...
        _origin = request.POST.get('origin_source')
        if not finding_id:
            return JsonResponse({"error": "Missing finding ID"}, status=400)

        try:
            # ✅ Generate unique filename
            filename = f"{uuid.uuid4()}.png"
            relative_path = os.path.join("uploads", "findings", str(finding_id), filename).replace("\\", "/")
            upload_dir = os.path.join(settings.MEDIA_ROOT, "uploads", "findings", str(finding_id))
